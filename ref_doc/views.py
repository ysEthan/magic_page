...
from .serializers import (
    ...
    StockInSerializer, StockOutSerializer, InventoryHistorySerializer
)
...
class InventoryHistoryFilter(filters.FilterSet):
    """库存变化明细过滤器"""
    start_date = filters.DateTimeFilter(field_name='operation_time', lookup_expr='gte')
    end_date = filters.DateTimeFilter(field_name='operation_time', lookup_expr='lte')
    product = filters.CharFilter(field_name='product__name', lookup_expr='icontains')
    sku = filters.CharFilter(field_name='product__code', lookup_expr='icontains')
...
class InventoryHistoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_class = InventoryHistoryFilter
    search_fields = ['product__name', 'product__code', 'source_number']
