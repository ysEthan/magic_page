    ...
    <el-card>
      ...
      <el-form :inline="true" :model="queryParams" class="search-form">
        ...
        <el-form-item label="出库类型">
          <el-select
            v-model="queryParams.stock_out_type"
            placeholder="请选择类型"
            clearable
            style="width: 150px"
          >
            <el-option
              v-for="(label, value) in stockOutTypeOptions"
              :key="value"
              :label="label"
              :value="value"
            />
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column prop="stock_out_code" label="出库单号" width="150" />
        ...
        <el-table-column label="出库类型" width="120">
          <template #default="{ row }">
            <el-tag>{{ row.stock_out_type_display }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="quantity" label="出库数量" width="100" />
        <el-table-column label="批次信息" width="150">
          <template #default="{ row }">
            {{ row.inventory?.batch_code }}
          </template>
        </el-table-column>
        <el-table-column prop="related_order" label="关联单号" width="150" show-overflow-tooltip />
        ...
        <el-table-column label="出库时间" width="180">
          <template #default="{ row }">
            {{ formatDateTime(row.stock_out_time) }}
          </template>
        </el-table-column>
      </el-table>
...
import { getStockOutList } from '@/api/storage'
...
watch(dateRange, (newVal) => {
  if (newVal) {
    queryParams.value.min_stock_out_time = newVal[0]
    queryParams.value.max_stock_out_time = newVal[1]
  } else {
    queryParams.value.min_stock_out_time = ''
    queryParams.value.max_stock_out_time = ''
  }
})
...
const getList = async () => {
  loading.value = true
  try {
    const { count, results } = await getStockOutList(queryParams.value)
    stockOutList.value = results
    total.value = count
  } catch (error) {
